...
| Railway/GDELT external acceptance | `railway-monitor/`, `src/gdelt_client.py`, `src/external_acceptance.py` | read-only health/Pages capture and callback | `external-acceptance-2026-08-22T1330.json` (latest; prior `1006`–`1300` retained) | Railway monitor remains healthy; Gmail is configuration-missing, GDELT is HTTP 429, the health callback is HTTP 403, and the runtime reports a legacy delivery-secret migration requirement; no production promotion | NEEDS_REVERIFY |
