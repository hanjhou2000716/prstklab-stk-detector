...
| External acceptance gate summary | `src/external_acceptance.py`, `docs/external-acceptance-gates.md`, `docs/external-observation-lineage.md` | yes | read-only Railway/Pages probe plus observation lineage comparison | redacted `gate_summary` evidence | operator evidence (not release data) | never sends | partially_integrated |
...
| External source email contract | `src/email_intelligence.py`, `src/gmail_ingress.py`, `src/external_source_parsers.py`, `src/creator_source_adapters.py`, `railway-monitor/gmail_ingress.py`, `railway-monitor/gmail_watch.py`, `railway-monitor/gmail_watch_service.py`, `railway-monitor/email_store.py`, `railway-monitor/email_router.py` | yes | bounded Railway ingress + parser/DLQ boundary; one canonical Gmail Watch manager with async compatibility adapter | sanitized EmailObservation + private cursor/DLQ | source health/pending reasons | no raw mail | partially_integrated |
...
| Railway health contract/history | `src/railway_health_contract.py`, `railway-monitor/app.py`, `railway-monitor/health_state.py`, `railway-monitor/health_history_store.py`, `railway-monitor/state_store_schema.py`, `railway-monitor/runtime_config.py`, `railway-monitor/health_contract.py` | yes | monitor health boundary + standalone heartbeat/Gmail projection + redacted runtime configuration + bounded cycle history restored from SQLite | bounded status/retry/heartbeat/configuration/history state | source health | observability only | partially_integrated |
| Railway Gmail runtime wiring | `railway-monitor/gmail_runtime.py`, `railway-monitor/gmail_ingress.py`, `railway-monitor/gmail_watch.py`, `railway-monitor/gmail_watch_service.py`, `railway-monitor/email_store.py` | yes | standalone configuration-to-ingress boundary plus async adapter delegated to `GmailWatchManager`; overlap audit enforced | redacted Gmail watch health | source health | not directly | partially_integrated |
...
| FinancialJuice priority delivery | `src/financialjuice_priority.py`, `src/financialjuice_notification.py`, `src/scheduled_delivery.py`, `src/event_ledger.py` | yes | qualifying reviewed observations use the release-gated scheduled `sendPhoto` path | notification key + redacted recipient receipts | release-bound external event | vendor-priority only; PRStK risk remains separate | production |
...
| News interest/ranking/dedup | `src/news_intelligence.py`, `src/risk_news.py`, `src/event_classifier.py` | yes | market snapshot | `news.intelligence` + shared `event_classification` | badges/relevance reasons/event category/reason | context only | production |
