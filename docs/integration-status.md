...
| Railway health contract/history | `src/railway_health_contract.py`, `railway-monitor/app.py`, `railway-monitor/health_state.py`, `railway-monitor/runtime_config.py`, `railway-monitor/health_contract.py` | yes | monitor health boundary + standalone heartbeat/Gmail projection + redacted runtime configuration + bounded cycle history | bounded status/retry/heartbeat/configuration/history state | source health | observability only | partially_integrated |
