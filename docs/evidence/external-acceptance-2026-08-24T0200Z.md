# External acceptance capture — 2026-08-24 02:00Z
...
- Blocking reason: `railway_gdelt:failed` (`HTTP_429`). GDELT remains a
  discovery source and is fail-closed; no alert was promoted from it.
...
- Pages manifest: `ready`; five declared artifacts and five verified hashes;
...
`DEBT-EXT-003` (live news freshness) open. A later qualifying observation must
be sanitized, release-bound, and delivered only through the existing
single-recipient acceptance path before these items can be locked.
