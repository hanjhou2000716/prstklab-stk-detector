...
- `RAILWAY_STATUS_SHARED_SECRET` is the canonical variable name.
- `DELIVERY_STATUS_SHARED_SECRET` remains a temporary migration fallback.
...
`tests/test_railway_runtime_config.py` covers canonical precedence, legacy
compatibility, missing configuration, blank values, and secret redaction.
The remainder of P0-26 (further extraction of the event/HTTP service and live
Railway acceptance) remains `NEEDS_REVERIFY`/external and is not claimed by
