...
| REQ-ADD-037 | photo smoke Chromium installation boundary | `.github/workflows/notify.yml`, `docs/req-add-037-photo-smoke-chromium-install.md` | stalled run 31826716530 cancelled; targeted workflow review; follow-up scoped smoke run required | renderer failure remains fail-closed; no photo is sent when install times out | NEEDS_REVERIFY |
...
| REG-009 | REQ-ADD-034 | GDELT HTTP 200 non-JSON response escaped the cache fallback and failed the poll cycle | response payload parsing occurred after the guarded fetch block | payload validation moved inside the guarded block; recent-cache fallback fixture; 86 targeted and 1227 full tests | CLOSED |
...
| DEBT-005 | Controlled Telegram delivery receipt and Mini App deep-link not captured | post-merge Railway health shows delivery `not_checked`; photo smoke run 31826716530 stalled at Chromium install | merge REQ-ADD-037 then rerun one approved recipient test | OPEN / EXTERNAL |
