...
| REQ-ADD-008 Railway classifier delivery gate | PASS (local) / NEEDS_REVERIFY (external) | `railway-monitor/app.py`, `src/event_classifier.py` | 84 Railway monitor tests; full regression pending on this atomic change; standalone import isolation proves fallback is diagnostics-only | non-canonical root-only classifier cannot create Jin10 dispatch; incoming item remains retryable for a correctly packaged deployment | Railway health must report repository-shared classifier before delivery | NEEDS_REVERIFY |
