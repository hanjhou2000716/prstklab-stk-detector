...
| HEAD at last evidence capture | `80ae76f` (`fix(REQ-ADD-008): gate Railway dispatch on canonical classifier`) |
| Recovery checkpoint | `checkpoint/migration-2026-08-14-current19` (post-task snapshot) |
...
| Remote PR | #585 is open on the merged #584 base; REQ-ADD-008 commit `80ae76f` is pushed and its required quality/security checks pass |
