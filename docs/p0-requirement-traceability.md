...
| P0-19 Gmail ingress | `railway-monitor/gmail_ingress.py`, `railway-monitor/email_store.py`, `railway-monitor/email_router.py`, `railway-monitor/storage_health.py`, `tests/test_p0_19_gmail_ingress_contract.py` | targeted 20 passed; identity-boundary regression in `tests/test_email_intelligence.py` and `tests/test_railway_gmail_gateway.py`; Gmail runtime/storage 8 passed; full branch regression 1,511 passed; CI run `31722239089` / job `94521788534` green; security/CodeQL/SBOM passed | authenticated bounded Pub/Sub; cursor-only persistence; replay-safe dedupe; malformed/configuration failures fail closed; body-only Creator markers cannot hijack routing; startup probe cannot stall on Windows/OneDrive | PASS → LOCKED (offline; live watch remains external) |
...
| P0-21 Mini App release/deep-link | `src/deep_link_router.py`, public loader | `tests/test_p0_21_mini_app_routing_contract.py`; 8 targeted after canonical identity fix; public base load and 360px no-overflow evidence in [`docs/evidence/miniapp-browser-acceptance-2026-08-29.json`](evidence/miniapp-browser-acceptance-2026-08-29.json) | release mismatch and missing alerts stay archived/missing; Pages must be reverified after the branch fix is deployed | NEEDS_REVERIFY (external) |
