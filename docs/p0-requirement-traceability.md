...
| P0-19 Gmail ingress | `railway-monitor/gmail_ingress.py`, `railway-monitor/email_store.py`, `railway-monitor/email_router.py`, `tests/test_p0_19_gmail_ingress_contract.py` | targeted 20 passed; identity-boundary regression in `tests/test_email_intelligence.py` and `tests/test_railway_gmail_gateway.py`; CI run `31722239089` / job `94521788534` green; security/CodeQL/SBOM passed | authenticated bounded Pub/Sub; cursor-only persistence; replay-safe dedupe; malformed/configuration failures fail closed; body-only Creator markers cannot hijack routing | PASS → LOCKED (offline; live watch remains external) |
