...
| P0-15 release freshness and stale quote gate | `src/market_data.py`, `src/release_gate.py`, `tests/test_p0_15_freshness_gate_contract.py` | targeted 72 passed with isolated basetemp; freshness, release manifest and strict gate regressions passed; CI evidence pending | stale/recent-close data remains visible but cannot alert; non-fresh research cannot pass strict release gate | NEEDS_REVERIFY |
