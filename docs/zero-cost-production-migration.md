...
Before cutover, verify a real Supabase job, a successful Actions run, Worker
health, Pages polling and one explicitly authorised Telegram canary. If any
gate fails, stop dispatching new jobs, restore the previous Pages release and
continue using the legacy Railway monitor. Do not delete Railway data until
the external acceptance evidence is archived.
