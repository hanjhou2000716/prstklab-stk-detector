...
const renderResearch = (snapshot) => {
  ...
  const backtestState = String(report.backtest_release_status || "").toLowerCase();
  const backtestNotice = backtestState === "unavailable" || backtestState === "blocked"
    ? "正式回測尚未可用；目前僅供研究觀察，不提供操作判斷。"
    : "";
