...
| 價格訊號 | 台指絕對變動達 1.5%、費半達 3%、Nasdaq 達 2%、WTI／Brent 達 5%；延遲報價不觸發 | 對應標的與兩個相關市場的最新可核對報價 |

同一外部事件依事件 ID 嚴格去重；同類後續消息預設 30 分鐘冷卻，只有內容明顯升級、官方資訊更新或市場波動擴大時才會提前再次發送。未達上述條件的商品、加密或一般市場消息只更新儀表板，不推播 Telegram。所有文字使用「已知事實、可能影響、持續觀察」結構，不提供買賣或配置指令。
