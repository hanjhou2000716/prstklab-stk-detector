...
每次成功完成的統一研究摘要與代表標的資料，以及每次快報更新的市場快照，都會保存為公開的 GitHub Pages 資料檔。所有 Pages 發布工作會依序執行，避免其他部署覆蓋最新資料；內容只包含公開市場研究結果，不包含帳戶、持倉或任何機敏資料。

### 重大事件手動備援

在 **Actions → Emergency market alert → Run workflow**，選擇事件類別並填寫中立摘要，即可發送少於 30 字的 Telegram 重大快訊與 Mini App 按鈕。這是金十等即時資料源尚未設定前的人工備援；不會自動把任何未驗證訊息推播出去。
