...
def _repository_dispatch_context(now: datetime, scheduled_for_at: str) -> dict[str, str] | None:
    from src.schedule_contract import SCHEDULE_CONTRACT_VERSION, timezone_for_slot

    anchor = datetime.fromisoformat(scheduled_for_at)
    return resolve_slot_context(
        "pre_open",
        now,
        trigger_kind="repository_dispatch",
        scheduled_for_at=scheduled_for_at,
        contract_version=SCHEDULE_CONTRACT_VERSION,
        time_zone=timezone_for_slot("pre_open"),
        dispatch_unix=str(int(anchor.timestamp())),
        dispatch_trace_id="test-slot-deadline",
    )


def test_repository_dispatch_delivery_window_is_half_open():
    from src.scheduled_delivery import _resolve_delivery_obligation

    scheduled = "2026-09-28T08:45:00+08:00"
    just_before = _repository_dispatch_context(
        datetime(2026, 9, 28, 9, 14, 59, tzinfo=ZoneInfo("Asia/Taipei")),
        scheduled,
    )
    assert just_before is not None
    assert just_before["delivery_intent"] == "notify_candidate"
    assert just_before["resolution_reason"] == "dispatch_anchor_on_time"

    exact_deadline = _repository_dispatch_context(
        datetime(2026, 9, 28, 9, 15, tzinfo=ZoneInfo("Asia/Taipei")),
        scheduled,
    )
    assert exact_deadline is not None
    assert exact_deadline["delivery_intent"] == "publish_only"
    assert exact_deadline["resolution_reason"] == "late_dispatch_publish_only"
    obligation, reason, _states = _resolve_delivery_obligation(
        {}, "pre_open", exact_deadline, notification_requested=True,
    )
    assert obligation == "late_publish_only"
    assert reason == "late_dispatch_publish_only"


def test_late_repository_dispatch_preserves_original_slot_date_across_midnight():
    scheduled = "2026-09-28T08:45:00+08:00"
    context = _repository_dispatch_context(
        datetime(2026, 9, 29, 6, 15, tzinfo=ZoneInfo("Asia/Taipei")),
        scheduled,
    )
    assert context is not None
    assert context["scheduled_slot"] == "pre_open"
    assert context["slot_date"] == "2026-09-28"
    assert context["scheduled_for_at"] == scheduled
    assert context["delivery_intent"] == "publish_only"
    assert context["resolution_reason"] == "late_dispatch_publish_only"
