-- Extend the private FJ recovery record from summary-only state to the full
-- delivery lifecycle.  No raw mail, address or Telegram identifier is stored.
alter table public.financialjuice_priority_pending
...
alter table public.financialjuice_priority_pending
  add column if not exists last_blocking_reason text;
...
alter table public.financialjuice_priority_pending
  alter column delivery_status set default 'summary_pending';
...
begin
  if not exists (
    select 1
      from pg_constraint
     where conname = 'financialjuice_priority_pending_delivery_status_check'
  ) then
    alter table public.financialjuice_priority_pending
      add constraint financialjuice_priority_pending_delivery_status_check
...
create index if not exists financialjuice_priority_pending_delivery_idx
  on public.financialjuice_priority_pending (delivery_status, next_retry_at, expires_at);
...
begin
  ...
  on conflict (canonical_fact_key, material_fact_version) do update set
    ...
    summary_status = case
      when public.financialjuice_priority_pending.delivery_status in ('delivered', 'expired', 'contract_failed')
...
begin
  if p_next_status not in (
    ...
    raise exception 'priority_delivery_status_invalid';
  end if;
