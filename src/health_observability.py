...
def aggregate_source_health(records: Iterable[dict[str, Any]]) -> dict[str, Any]:
    rows = list(records)
    total = len(rows)
    successful = sum(str(row.get("status", "")).lower() in {"ok", "healthy", "success"} for row in rows)
    ...
    return {
        ...
        "state": "healthy" if total and failures == 0 else "partial" if successful else "failed" if total else "no_observations",
    }
