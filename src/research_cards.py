...
def load_research_cards(path: Path = REPORT_PATH, *, now: datetime | None = None) -> dict[str, Any]:
    ...
    for item in raw.get("candidates", []):
        ...
        candidate = {key: item.get(key) for key in (
            ...
            "heat_verified", "financial_complete", "financial_status", "eps_ytd",
            "total_net_income_ytd", "total_equity", "annualized_quality_ratio",
            "quality_basis", "calculation_basis", "financial_period",
            "verification_gaps", "passed_conditions", "failed_conditions",
