...
def parse_tpex_index(payload: Any) -> dict[str, Any] | None:
    ...
    change = round(close - prior_close, 2) if prior_close is not None and prior_close != 0 else None
    change_percent = round((close / prior_close - 1) * 100, 2) if prior_close is not None and prior_close != 0 else None
    return {
        "symbol": "^TWOII",
        "ticker": "TPEx",
        "name": "臺灣櫃買指數",
...
def parse_twse_mis_tpex(payload: Any) -> dict[str, Any] | None:
    ...
    return {
        ...
        "change": round(close - previous, 2) if previous is not None and previous != 0 else None,
        "change_percent": round((close / previous - 1) * 100, 2) if previous is not None and previous != 0 else None,
...
def _recent_close_record(
    ...
    *,
    ...
    record: dict[str, Any] = {
        ...
        "previous_close": round(previous, 2) if previous is not None else None,
        "change": round(latest - previous, 2) if previous is not None and previous != 0 else None,
        "change_percent": round((latest / previous - 1) * 100, 2) if previous is not None and previous != 0 else None,
        "quote_date": quote_date,
        "quote_time": None,
        "quote_source": source,
        "quote_basis": basis,
