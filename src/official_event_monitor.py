...
def select_official_event(snapshot: dict[str, Any]) -> dict[str, str] | None:
    """Select one current official release; other event sources never trigger this monitor."""
    items = snapshot.get("official_events", {}).get("items", [])
    return items[0] if items else None


def event_key(event: dict[str, str] | None) -> str:
    """Create a stable idempotency key from source facts, not a rendered message."""
    if not event:
        return "none"
    material = "|".join(event.get(key, "") for key in ("url", "title", "released_at"))
    return hashlib.sha256(material.encode("utf-8")).hexdigest()[:24]


def build_official_event_brief(event: dict[str, str]) -> str:
    """Make a neutral watch-sized alert while retaining the official category."""
...
def prepare_snapshot() -> tuple[dict[str, Any], dict[str, str] | None]:
...
def write_status_output(event: dict[str, str] | None) -> None:
...
def send_current_event(expected_key: str | None = None) -> None:
    """Send one fresh official event, refusing a changed event between workflow steps."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="監測 Fed、BLS、BEA 官方重大事件")
